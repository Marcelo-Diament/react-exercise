...
const getCurrTime = () => {
  return `${new Date().getUTCFullYear()}${new Date().getUTCMonth()}${new Date().getUTCDay()}_${new Date().toLocaleTimeString()}`
}
...
const pushDataLayer = data => {
  ...
  if (data.accessDate !== data.lastAccessDate) {
...
export { dataLayerInit, getDataLayer, updateDataLayer }